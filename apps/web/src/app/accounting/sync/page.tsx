...
type StateFilter = 'ALL' | 'FAILED' | 'PENDING' | 'IN_PROGRESS' | 'SYNCED' | 'CANCELLED' | 'SKIPPED';
...
const STATE_FILTERS: StateFilter[] = ['ALL', 'FAILED', 'PENDING', 'IN_PROGRESS', 'SYNCED', 'CANCELLED', 'SKIPPED'];
const CUSTOMER_STATE_FILTERS: StateFilter[] = ['ALL', 'FAILED', 'PENDING', 'IN_PROGRESS', 'SYNCED', 'SKIPPED'];
...
export default function SyncMonitorPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    const invoiceRequests = INVOICE_LOAD_STATES.map((syncState) => listInvoiceSyncRecords({ state: syncState, limit: 200 }));
    const customerRequests = CUSTOMER_LOAD_STATES.map((syncState) => listCustomerSyncs({ state: syncState, limit: 200 }));
    Promise.allSettled([
      ...invoiceRequests,
      ...customerRequests,
      listIntegrationAccounts(),
      ...
      const invoiceResults = results.slice(0, invoiceRequestCount) as PromiseSettledResult<{ items: InvoiceSyncRecord[] }>[];
      const customerResults = results.slice(invoiceRequestCount, invoiceRequestCount + customerRequestCount) as PromiseSettledResult<{ items: CustomerSyncRecord[] }>[];
      const [accountsResult, failureResult, statusResult] = results.slice(invoiceRequestCount + customerRequestCount) as [
      ...
      const invoiceGroups = invoiceResults
        .filter((r): r is PromiseFulfilledResult<{ items: InvoiceSyncRecord[] }> => r.status === 'fulfilled')
        .map(r => r.value.items);
      const customerGroups = customerResults
        .filter((r): r is PromiseFulfilledResult<{ items: CustomerSyncRecord[] }> => r.status === 'fulfilled')
        .map(r => r.value.items);
  ...
  async function retry(id: string) {
    setRetrying(id);
    setInvoiceRecords(prev => prev.map(r => r.id === id ? { ...r, state: 'IN_PROGRESS' as const } : r));
    try {
      await retryInvoiceSync(id);
      setInvoiceRecords(prev => prev.map(r => r.id === id ? { ...r, state: 'PENDING' as const } : r));
      toast.success('Retry queued');
    } catch (err) {
      setInvoiceRecords(prev => prev.map(r => r.id === id ? { ...r, state: 'FAILED' as const } : r));
  ...
  function updateLocation(next: { view?: SyncView; state?: StateFilter; period?: PeriodFilter }) {
    ...
    window.history.replaceState(null, '', `/accounting/sync?${qs.toString()}`);
  }
  ...
  const rows = useMemo(() => {
    ...
    const invoiceQueue = invoiceRecords.filter(r => r.state === 'PENDING' || r.state === 'IN_PROGRESS');
    const customerQueue = customerRecords.filter(r => r.state === 'PENDING' || r.state === 'IN_PROGRESS');
  ...
  const activeDescription =
    view === 'failures' ? 'Invoice and customer sync failures with retry context.'
    : view === 'queue' ? 'Records waiting to sync or currently in progress.'
    : view === 'invoices' ? 'Invoice sync history from the ERP to QuickBooks.'
    : view === 'customers' ? 'Customer sync history from the ERP to QuickBooks.'
    ...
    <div>
      ...
      {loadError && (
        <div className="mb-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <p className="text-sm font-semibold text-yellow-800">Could not load all sync data: {loadError}</p>
        </div>
      )}
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between gap-2">
                ...
                <span className={`rounded-full px-2 py-0.5 text-xs ${active ? 'bg-white/15 text-white' : 'bg-gray-100 text-gray-700'}`}>{count}</span>
              </div>
              <div className={`mt-1 text-[11px] ${active ? 'text-white/70' : 'text-gray-500'}`}>{item.description}</div>
            </button>
          );
        })}
      </div>
...
function FailureList({
  invoiceRows, customerRows, paymentFailureCount, retrying, onRetry,
}: {
  ...
  if (invoiceRows.length === 0 && customerRows.length === 0 && paymentFailureCount === 0) {
    return <EmptyState icon="OK" title="No sync failures" description="Invoice and customer sync records are healthy." />;
  }
  return (
    <div className="space-y-4">
      {paymentFailureCount > 0 && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
          {paymentFailureCount} payment failure{paymentFailureCount === 1 ? '' : 's'} are counted in the summary. Row-level payment retry is not exposed in this monitor yet.
        </div>
      )}
...
function QueueList({
  invoiceRows, customerRows,
}: {
  invoiceRows: InvoiceSyncRecord[];
  customerRows: CustomerSyncRecord[];
}) {
  if (invoiceRows.length === 0 && customerRows.length === 0) {
    return <EmptyState icon="OK" title="Queue is empty" description="No invoice or customer records are waiting to sync." />;
  }
  return (
    <div className="space-y-4">
      {invoiceRows.length > 0 && (
...
function InvoiceTable({
  records, retrying, onRetry, compact,
}: {
  ...
  if (records.length === 0) {
    return <EmptyState icon="OK" title="No invoice records" description="No invoices match this filter." />;
  }
  return (
      ...
      <table className="w-full text-sm">
        <thead className="bg-gray-50 border-b border-gray-200">
          <tr>
            ...
            {!compact && <th className="px-4 py-3 text-left font-medium text-gray-600">Work order</th>}
          ...
          {records.map(r => (
            <tr key={r.id} className="hover:bg-gray-50">
              <td className="px-4 py-3">
                <div className="font-mono text-xs font-semibold text-gray-900">{r.invoiceNumber}</div>
                <div className="text-[11px] text-gray-400">{r.externalReference ?? r.id}</div>
              </td>
              {!compact && <td className="px-4 py-3 font-mono text-xs text-gray-500">{r.workOrderId}</td>}
              <td className="px-4 py-3"><SyncState state={r.state} /></td>
              <td className="px-4 py-3 text-gray-500">{r.attemptCount}</td>
              <td className="px-4 py-3 text-xs text-gray-500">{formatDateTime(r.syncedAt ?? r.createdAt)}</td>
              <td className="px-4 py-3 text-xs text-red-600 max-w-xs truncate">{r.lastErrorMessage ?? r.lastErrorCode ?? '-'}</td>
...
function CustomerTable({ records, compact }: { records: CustomerSyncRecord[]; compact?: boolean }) {
  if (records.length === 0) {
    return <EmptyState icon="OK" title="No customer records" description="No customer sync records match this filter." />;
  }
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
      <table className="w-full text-sm">
        <thead className="bg-gray-50 border-b border-gray-200">
          <tr>
            ...
            {!compact && <th className="px-4 py-3 text-left font-medium text-gray-600">Provider</th>}
            <th className="px-4 py-3 text-left font-medium text-gray-600">Status</th>
            <th className="px-4 py-3 text-left font-medium text-gray-600">Retries</th>
            <th className="px-4 py-3 text-left font-medium text-gray-600">Synced</th>
            <th className="px-4 py-3 text-left font-medium text-gray-600">Error</th>
...
function AccountsTable({ accounts }: { accounts: IntegrationAccount[] }) {
  if (accounts.length === 0) {
    return <EmptyState icon="QB" title="No integration accounts" description="Connect QuickBooks or run account import to populate mappings." />;
  }
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-gray-100">
          {accounts.map(a => (
            <tr key={a.id} className="hover:bg-gray-50">
              <td className="px-4 py-3 font-semibold text-gray-900">{a.displayName ?? a.name ?? a.id}</td>
              ...
              <td className="px-4 py-3 font-mono text-xs text-gray-500">{a.accountKey ?? a.qbId ?? '-'}</td>
              <td className="px-4 py-3 text-xs text-gray-500">{formatDateTime(a.lastSyncedAt ?? a.updatedAt ?? a.createdAt)}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function SyncState({ state }: { state: string }) {
  if (state === 'PENDING' || state === 'IN_PROGRESS' || state === 'SYNCED' || state === 'FAILED' || state === 'RETRY') {
...
function normalizeView(raw: string | null): SyncView {
  if (raw === 'queue' || raw === 'invoices' || raw === 'customers' || raw === 'accounts' || raw === 'failures') {
    return raw;
  }
  return 'failures';
}
...
function normalizeState(raw: string | null): StateFilter {
  if (raw === 'FAILED' || raw === 'PENDING' || raw === 'IN_PROGRESS' || raw === 'SYNCED' || raw === 'CANCELLED' || raw === 'SKIPPED') {
    return raw;
  }
  return 'ALL';
}

function mergeById<T extends { id: string; createdAt?: string; syncedAt?: string | null }>(groups: T[][]): T[] {
