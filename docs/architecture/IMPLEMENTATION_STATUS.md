...
| SOP and OJT route/service contracts | SOP/OJT | ✅ Done | Team | Runtime route families in `apps/api/src/lambda/sop` and server wiring; training state coverage in `apps/api/src/tests/sop-training-state.test.ts`; repaired training UI, live API calls, and manager assignment creation in the web app | None for current runtime contracts | Add content authoring/editing once those APIs are designed |
