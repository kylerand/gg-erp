...
async function route(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  const purchaseOrderActionMatch = pathname.match(
  ...
  const woOperationStateMatch = pathname.match(
    /^\/tickets\/work-orders\/([^/]+)\/operations\/([^/]+)\/state$/,
  );
  const qcMatch = pathname.match(/^\/tickets\/work-orders\/([^/]+)\/qc-gates/);
  const timeEntryMatch = pathname.match(
    /^\/tickets\/work-orders\/([^/]+)\/time-entries(?:\/([^/]+))?/,
  );
  ...
  const commMessageReactionsMatch = pathname.match(
    /^\/communication\/messages\/([^/]+)\/reactions(?:\/([^/]+))?/,
  );
  ...
  const reportSubscriptionMatch = pathname.match(/^\/reporting\/subscriptions\/([^/]+)$/);
  ...
  if (pathname === '/auth/me' && method === 'GET') {
  ...
  } else if (pathname === '/reporting/snapshot' && method === 'GET') {
  ...
  } else if (pathname === '/reporting/subscriptions' && method === 'GET') {
    result = await listReportSubscriptionsHandler(event);
  } else if (pathname === '/reporting/subscriptions' && method === 'POST') {
    result = await createReportSubscriptionHandler(event);
  } else if (reportSubscriptionMatch && method === 'PATCH') {
    result = await updateReportSubscriptionHandler({
      ...event,
      pathParameters: {
        ...event.pathParameters,
        subscriptionId: reportSubscriptionMatch[1],
      },
    });
  } else if (pathname === '/reporting/exports' && method === 'GET') {
    result = await listReportExportRunsHandler(event);
  } else if (pathname === '/reporting/exports/run-now' && method === 'POST') {
    result = await runReportExportNowHandler(event);
    ...
    result = await transitionCustomerStateHandler({
      ...event,
      pathParameters: { id: customerMatch[1] },
    });
    ...
    inventoryReservationMatch[2] === 'release' &&
    method === 'PATCH'
  ) {
    result = await releaseReservationHandler({
      ...event,
      pathParameters: { id: inventoryReservationMatch[1] },
    });
  } else if (
    inventoryReservationMatch &&
    inventoryReservationMatch[2] === 'consume' &&
    method === 'PATCH'
  ) {
    result = await consumeReservationHandler({
      ...event,
      pathParameters: { id: inventoryReservationMatch[1] },
    });
    ...
    result = await getPurchaseOrderHandler({
      ...event,
      pathParameters: { id: purchaseOrderMatch[1] },
    });
  } else if (purchaseOrderMatch && method === 'PATCH') {
    result = await updatePurchaseOrderHandler({
      ...event,
      pathParameters: { id: purchaseOrderMatch[1] },
    });
  } else if (
    purchaseOrderActionMatch &&
    purchaseOrderActionMatch[2] === 'approve' &&
    method === 'PATCH'
  ) {
    result = await approvePurchaseOrderHandler({
      ...event,
      pathParameters: { id: purchaseOrderActionMatch[1] },
    });
  } else if (
    purchaseOrderActionMatch &&
    purchaseOrderActionMatch[2] === 'send' &&
    method === 'PATCH'
  ) {
    result = await sendPurchaseOrderHandler({
      ...event,
      pathParameters: { id: purchaseOrderActionMatch[1] },
    });
  } else if (
    purchaseOrderActionMatch &&
    purchaseOrderActionMatch[2] === 'cancel' &&
    method === 'PATCH'
  ) {
    result = await cancelPurchaseOrderHandler({
      ...event,
      pathParameters: { id: purchaseOrderActionMatch[1] },
    });
  } else if (
    purchaseOrderActionMatch &&
    purchaseOrderActionMatch[2] === 'close' &&
    method === 'PATCH'
  ) {
    result = await closePurchaseOrderHandler({
      ...event,
      pathParameters: { id: purchaseOrderActionMatch[1] },
    });
    ...
    result = await transitionTaskHandler({
      ...event,
      pathParameters: { workOrderId: taskMatch[1], id: taskMatch[2] },
    });
    ...
    result = await createReworkHandler({
      ...event,
      pathParameters: { workOrderId: reworkMatch[1] },
    });
  } else if (woOperationStateMatch && method === 'PATCH') {
    result = await transitionWoOperationHandler({
      ...event,
      pathParameters: {
        workOrderId: woOperationStateMatch[1],
        operationId: woOperationStateMatch[2],
      },
    });
    ...
    result = await batchSubmitQcGatesHandler({
      ...event,
      pathParameters: { workOrderId: qcMatch[1] },
    });
  } else if (timeEntryMatch && method === 'GET' && !timeEntryMatch[2]) {
    result = await listTimeEntriesHandler({
      ...event,
      pathParameters: { workOrderId: timeEntryMatch[1] },
    });
  } else if (timeEntryMatch && method === 'POST' && !timeEntryMatch[2]) {
    result = await createTimeEntryHandler({
      ...event,
      pathParameters: { workOrderId: timeEntryMatch[1] },
    });
  } else if (timeEntryMatch && timeEntryMatch[2] && method === 'PATCH') {
    result = await updateTimeEntryHandler({
      ...event,
      pathParameters: { workOrderId: timeEntryMatch[1], id: timeEntryMatch[2] },
    });
  } else if (timeEntryMatch && timeEntryMatch[2] && method === 'DELETE') {
    result = await deleteTimeEntryHandler({
      ...event,
      pathParameters: { workOrderId: timeEntryMatch[1], id: timeEntryMatch[2] },
    });
    ...
    result = await updateTimeEntryHandler({
      ...event,
      pathParameters: { id: flatTimeEntryMatch[1] },
    });
    ...
    result = await transitionTaskHandler({
      ...event,
      pathParameters: { id: technicianTaskMatch[1] },
    });
    ...
    result = await transitionRoutingStepStateHandler({
      ...event,
      pathParameters: { id: routingStepStateMatch[1] },
    });
    ...
    result = await listRoutingStepsHandler({
      ...
      queryStringParameters: {
        ...
        workOrderId: legacyRoutingStepMatch[1],
      },
    });
  } else if (legacyRoutingStepMatch && method === 'PATCH') {
    result = await transitionRoutingStepStateHandler({
      ...event,
      pathParameters: { id: legacyRoutingStepMatch[1] },
    });
    ...
    result = await getModuleProgressHandler({
      ...event,
      pathParameters: { id: sopModuleProgressMatch[1], employeeId: sopModuleProgressMatch[2] },
    });
  } else if (sopModuleMatch && pathname.endsWith('/step-progress') && method === 'PUT') {
    result = await updateStepProgressHandler({
      ...event,
      pathParameters: { id: sopModuleMatch[1] },
    });
    ...
    result = await getTrainingModuleHandler({
      ...event,
      pathParameters: { id: sopModuleMatch[1] },
    });
    // Notes
    ...
    result = await deleteNoteHandler({
      ...event,
      pathParameters: { id: pathname.split('/sop/notes/')[1] },
    });
    // Bookmarks
    ...
    result = await answerQuestionHandler({
      ...event,
      pathParameters: { id: sopQuestionAnswerMatch[1] },
    });
    // Assignments
    ...
    result = await completeAssignmentHandler({
      ...event,
      pathParameters: { assignmentId: sopMatch[1] },
    });
    // Inspection Templates (must come before generic sopMatch GET)
  } else if (sopItMatch && method === 'GET' && sopItMatch[1]) {
    result = await getInspectionTemplateHandler({
      ...event,
      pathParameters: { id: sopItMatch[1] },
    });
    ...
    (pathname === '/accounting/oauth/connect' || pathname === '/accounting/quickbooks/connect') &&
    method === 'GET'
  ) {
    result = await oauthConnectHandler(event);
  } else if (
    (pathname === '/accounting/oauth/callback' || pathname === '/accounting/quickbooks/callback') &&
    method === 'GET'
  ) {
    result = await oauthCallbackHandler(event);
  } else if (
    (pathname === '/accounting/status' || pathname === '/accounting/quickbooks/status') &&
    method === 'GET'
  ) {
    result = await qbStatusHandler(event);
  } else if (
    (pathname === '/accounting/invoice-sync' || pathname === '/accounting/invoices') &&
    method === 'GET'
  ) {
    ...
    result = await retryInvoiceSyncHandler({
      ...event,
      pathParameters: { id: qbInvoiceSyncMatch[1] },
    });
    ...
    (pathname === '/accounting/payment-syncs' || pathname === '/accounting/payments') &&
    method === 'GET'
  ) {
    result = await listPaymentSyncsHandler(event);
  } else if (qbPaymentSyncMatch && pathname.endsWith('/retry') && method === 'POST') {
    result = await retryPaymentSyncHandler({
      ...event,
      pathParameters: { id: qbPaymentSyncMatch[1] },
    });
    ...
    result = await getReconciliationRunHandler({
      ...event,
      pathParameters: { id: qbReconciliationRunMatch[1] },
    });
    ...
    result = await resolveReconciliationHandler({
      ...event,
      pathParameters: { id: qbReconciliationRecordMatch[1] },
    });
    ...
    result = await updateAccountStatusHandler({
      ...event,
      pathParameters: { id: qbIntegrationAccountMatch[1] },
    });
    ...
    result = await adminUpdateUserHandler({
      ...event,
      pathParameters: { username: adminUserMatch[1] },
    });
  } else if (adminUserMatch && method === 'DELETE') {
    result = await adminDeleteUserHandler({
      ...event,
      pathParameters: { username: adminUserMatch[1] },
    });
    ...
    result = await listMessagesHandler({
      ...event,
      pathParameters: { channelId: channelMessagesMatch[1] },
    });
    ...
    result = await listRepliesHandler({
      ...event,
      pathParameters: { messageId: commMessageRepliesMatch[1] },
    });
  } else if (commMessageReactionsMatch && method === 'POST') {
    result = await addReactionHandler({
      ...event,
      pathParameters: { messageId: commMessageReactionsMatch[1] },
    });
  } else if (commMessageReactionsMatch && commMessageReactionsMatch[2] && method === 'DELETE') {
    result = await removeReactionHandler({
      ...event,
      pathParameters: {
        messageId: commMessageReactionsMatch[1],
        emoji: commMessageReactionsMatch[2],
      },
    });
  } else if (commMessageMatch && method === 'PATCH') {
    result = await editMessageHandler({
      ...event,
      pathParameters: { messageId: commMessageMatch[1] },
    });
    ...
    result = await listTodosHandler({
      ...event,
      pathParameters: { channelId: channelTodosMatch[1] },
    });
  } else if (channelTodosMatch && method === 'POST') {
    result = await createTodoHandler({
      ...event,
      pathParameters: { channelId: channelTodosMatch[1] },
    });
    ...
    result = await transitionOpportunityStageHandler({
      ...event,
      pathParameters: { id: salesOpportunityMatch[1] },
    });
  } else if (salesOpportunityMatch && method === 'GET') {
    result = await getOpportunityHandler({
      ...event,
      pathParameters: { id: salesOpportunityMatch[1] },
    });
  } else if (salesOpportunityMatch && method === 'PATCH') {
    result = await updateOpportunityHandler({
      ...event,
      pathParameters: { id: salesOpportunityMatch[1] },
    });
    ...
    result = await updateQuoteLinesHandler({
      ...event,
      pathParameters: { id: salesQuoteMatch[1] },
    });
    ...
    result = await convertQuoteToWorkOrderHandler({
      ...event,
      pathParameters: { id: salesQuoteMatch[1] },
    });
    ...
    result = await salesAgentSessionDetailHandler({
      ...event,
      pathParameters: { sessionId: salesAgentSessionMatch[1] },
    });
    ...
    result = await copilotSessionDetailHandler({
      ...event,
      pathParameters: { sessionId: copilotSessionMatch[1] },
    });
    ...
    result = await updateCapacitySlotHandler({
      ...event,
      pathParameters: { id: capacitySlotMatch[1] },
    });
  } else if (capacitySlotMatch && capacitySlotMatch[2] === 'cancel' && method === 'POST') {
    result = await cancelCapacitySlotHandler({
      ...event,
      pathParameters: { id: capacitySlotMatch[1] },
    });
    ...
    result = await completeAssignmentHandler({
      ...event,
      pathParameters: { assignmentId: ojtAssignmentMatch[1] },
    });
