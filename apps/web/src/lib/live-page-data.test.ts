...
test('customer dealer ops views are backed by live customer and cart data', () => {
  ...
  assert.deepEqual(
    [
      ...
      "searchParams.get('create') === 'relationship'",
  ...
  assert.deepEqual(
    [
      ...
      'export const createDealerAccountHandler',
      'getPrisma().dealerAccount.create',
      'export const updateDealerAccountHandler',
      'getPrisma().dealerAccount.update',
...
test('inventory procurement drill-in uses live PO/vendor reads and focused receiving links', () => {
  ...
  assert.deepEqual(
    [
      ...
      'buildPartExceptionHandoffRow',
      'selectedMissingCostCount',
      'selectedReorderCount',
      'selectedStockAuditCount',
      'selectedExceptionHandoffRows',
      'Selected Exception Workbench',
      'copySelectedActionBrief',
      'exportSelectedExceptionHandoffs',
      'Copy action brief',
      'Export handoffs',
      'First ledger',
      'First count',
      'First adjust',
      'First PO',
      "erpRoute('inventory-ledger', { partId: p.id })",
      "erpRoute('cycle-count', {",
      "erpRoute('inventory-adjustment', {",
      'createPartId: p.id',
      'createPartSku: p.sku',
      'formatHandoffQuantity(p.shortfallQuantity)',
  ...
  assert.deepEqual(
    [
      ...
      "erpRecordRoute('part', activePartId)",
