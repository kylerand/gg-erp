...
export default function MyQueuePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setTasks(
        Array.isArray(items) ? items.filter((t): t is TechnicianTask => !!t && !!t.state) : [],
      );
  ...
  async function handleBlock(taskId: string) {
    ...
    const prevTask = { ...task };
    const reasonLabel =
      BLOCK_REASON_OPTIONS.find((r) => r.value === blockReasonCode)?.label ?? blockReasonCode;
    ...
    <div className="space-y-4">
      ...
      <Dialog
        ...
        onOpenChange={(open: boolean) => {
          if (!open) setBlockDialogTaskId(null);
        }}
      >
        <DialogContent>
          <DialogHeader>
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(v) => {
                    if (v) setBlockReasonCode(v as BlockedReasonCode);
                  }}
                >
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => {
                if (blockDialogTaskId) void handleBlock(blockDialogTaskId);
              }}
...
function TaskCard({
  ...
  const blockedReasonLabel =
    ...
    (task.blockedReasonCode
      ? (BLOCK_REASON_OPTIONS.find((r) => r.value === task.blockedReasonCode)?.label ??
        task.blockedReasonCode)
      : null);
      ...
      <div className="flex items-start gap-4">
        ...
        {!readOnly && (
          <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
            {busy && (
              <span
                className="text-xs text-gray-400 animate-pulse w-5 text-center"
