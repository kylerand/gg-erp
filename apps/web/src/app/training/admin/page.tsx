  ...
  return (
    <div>
      <PageHeader
        ...
        description="Manage modules and review trainee progress"
      />
              ...
              <div key={m.id} className="flex items-center gap-4 px-5 py-3 hover:bg-gray-50">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-sm text-gray-900 truncate">{m.moduleName}</span>
                    {m.requiresSupervisorSignoff && (
                      <span className="text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded">Sign-off</span>
                    )}
                    <span className={`text-xs px-1.5 py-0.5 rounded ${
                      m.moduleStatus === 'ACTIVE' ? 'bg-green-100 text-green-700' :
                      m.moduleStatus === 'INACTIVE' ? 'bg-gray-100 text-gray-600' :
