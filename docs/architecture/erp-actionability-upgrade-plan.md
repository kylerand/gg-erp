...
| Unified work-order command center  | In Progress | `/work-orders/[id]` route with services, parts, time, QC, SOP, messages, accounting links, live material quantities, inline reserve/release/fulfill actions, labor logging, QC submit, resolved customer/cart drawers, quote and opportunity links, and richer derived activity | Add editable service operation state transitions and formal quote conversion once those APIs exist      |
...
| Inventory and purchasing workflows | In Progress | PO receiving, PO/vendor list and detail reads, purchase-order drill-in, focused receiving links, PO create/edit/approve/send/cancel/close actions, work-order reservation execution, live replenishment recommendations with draft PO creation, receiving variance reporting with accepted/rejected quantity posting, and live accounting payable review for received POs are wired to live inventory APIs | Add import/export and broader row actions                                                              |
