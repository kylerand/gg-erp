  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Work Orders', value: total, sub: `${blocked.length} blocked`, href: '/work-orders', color: blocked.length > 0 ? 'text-red-600' : 'text-gray-900' },
          { label: 'In Progress', value: items.filter(w => w.status === 'IN_PROGRESS').length, sub: 'active builds', href: '/work-orders/dispatch', color: 'text-yellow-700' },
          { label: 'Completed', value: items.filter(w => w.status === 'COMPLETED').length, sub: 'this period', href: '/work-orders', color: 'text-green-700' },
        ].map(stat => (
