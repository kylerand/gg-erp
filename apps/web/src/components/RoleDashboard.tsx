  ...
  technician: [
    {
      ...
      title: 'Blocked Jobs',
      value: 1,
      description: 'Active jobs blocked on parts',
    ...
    },
    {
      ...
      value: 1,
      description: 'OJT modules past due date',
      href: erpRoute('my-ojt'),
      icon: '📚',
    },
    {
      id: 't4',
      ...
      value: 2,
      description: 'Work orders finished today',
  ...
  manager: [
    {
      ...
      title: 'Blocked Work',
      value: 2,
      description: 'Work orders needing triage',
      href: erpRoute('blocked-work'),
      icon: '🚧',
      alert: true,
    },
    {
      ...
      value: 5,
      description: 'Unassigned work orders',
    ...
    },
    {
      ...
      title: 'Overdue OJT',
      value: 1,
      description: 'Team members with overdue training',
    ...
    },
    {
      ...
      title: 'Throughput',
      value: '87%',
      description: 'Slot utilization this week',
  ...
  parts: [
    {
      ...
      title: 'Shortages',
      value: 1,
      description: 'Active reservations with no stock',
    ...
    },
    {
      ...
      title: 'Out of Stock',
      value: 1,
      description: 'SKUs at zero quantity',
      href: erpRoute('part'),
      icon: '📦',
    },
    {
      ...
      title: 'Pending Picks',
      value: 2,
      description: 'Reservations not yet picked',
      href: erpRoute('inventory-reservation'),
      icon: '📋',
    },
  ],
  trainer: [
    {
      ...
      value: 1,
      description: 'Team members past due date',
      href: erpRoute('training-assignment'),
      icon: '⚠️',
      alert: true,
    },
    {
      ...
      title: 'In Progress',
      value: 3,
      description: 'Modules actively being completed',
      href: erpRoute('training-assignment'),
      icon: '🔄',
    },
    {
      ...
      value: 2,
      description: 'Submitted evidence awaiting review',
      href: erpRoute('training-assignment'),
      icon: '📎',
    },
    {
      ...
      title: 'SOP Drafts',
      value: 1,
      description: 'SOPs pending review and publish',
      href: erpRoute('sop-library'),
      icon: '📖',
    },
  ],
  accounting: [
    // Static values are 0; the real counts are filled in by fetchCounts → VALUE_OVERRIDES.
    {
      ...
      title: 'QB Connection',
      value: '…',
      description: 'QuickBooks integration status',
    ...
    },
    {
      ...
      title: 'Sync Failures',
      value: 0,
      description: 'Invoice + customer + payment failures',
    ...
    },
    {
      id: 'ac2',
      priority: 'p2',
      title: 'Latest Reconcile',
      value: '—',
      description: 'Most recent reconciliation run',
    ...
    },
    {
      ...
      title: 'Pending Sync',
      value: 0,
      description: 'Records queued for sync',
      href: erpRoute('accounting-sync', { view: 'queue' }),
      icon: '⏳',
    },
    {
      id: 'ac4',
      ...
      title: 'Synced Today',
      value: 0,
      description: 'Invoices synced in last 24h',
      href: erpRoute('accounting-sync', { view: 'invoices', state: 'SYNCED', period: 'today' }),
      icon: '✅',
    },
  ],
  admin: [
    {
      ...
      title: 'Auth Failures',
      value: 1,
      description: 'Denied access attempts in last 24h',
...
const DEFAULT_ROLE_LANDING: Record<UserRole, string> = {
  technician: erpRoute('my-work-queue'),
  manager: erpRoute('dispatch-board'),
  parts: erpRoute('inventory-reservation'),
  trainer: erpRoute('training-assignment'),
  accounting: erpRoute('accounting-reconciliation'),
  admin: erpRoute('admin'),
};
    ...
    async function fetchCounts() {
      try {
        ...
        const completedToday =
          woCompleted.status === 'fulfilled'
            ? woCompleted.value.items.filter(
                (w) => w.completedAt && new Date(w.completedAt).toDateString() === today,
              ).length
            : 0;
        const auditEventsToday =
          auditAll.status === 'fulfilled'
            ? auditAll.value.items.filter((e) => new Date(e.createdAt).toDateString() === today)
                .length
            : 0;
        ...
        const totalSyncFailures =
          qbFailureSummary.status === 'fulfilled' ? qbFailureSummary.value.total : syncFailures;
        const invoicesSyncedToday =
          qbInvoiceSynced.status === 'fulfilled'
            ? qbInvoiceSynced.value.items.filter(
                (r) => r.syncedAt && new Date(r.syncedAt).toDateString() === today,
              ).length
            : 0;
        const lastReconLabel =
          qbReconRuns.status === 'fulfilled' && qbReconRuns.value.items[0]
  ...
  return (
    <div>
      <div className="mb-8 brand-panel p-6">
        <div className="brand-pill border-[#F6D1B7] bg-[#FFF3E8] text-[#8A4A18]">
          Today at a glance
        </div>
        ...
        <Link
          href={landing}
          className="inline-block mt-4 text-sm text-[#B1581B] hover:text-[#8A4A18] font-semibold hover:underline"
        >
          Go to my workspace
        </Link>
      </div>
...
function TodayQueue({
  ...
  today: WorkspaceTodayResponse | null;
  className?: string;
}) {
  ...
  return (
    <section className={`brand-panel p-5 ${className ?? ''}`}>
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <div className="brand-pill border-[#D9CCBE] bg-white text-[#6E625A]">
            Operator worklist
          </div>
          <h2 className="mt-3 text-2xl text-[#211F1E]" data-brand-heading="true">
...
function TodayQueueItem({ item }: { item: WorkspaceTodayItem }) {
  ...
  return (
    <Link
      ...
      className="grid gap-3 px-4 py-3 transition-colors hover:bg-[#FFF7EE] md:grid-cols-[7rem_1fr_auto] md:items-center"
    >
      <div
        className={`inline-flex h-8 w-20 items-center justify-center rounded-lg border text-xs font-bold ${cfg.classes}`}
      >
        {item.severity}
      </div>
      <div>
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-2">
        ...
        <span className="text-xs font-semibold opacity-60" aria-hidden>
          {cfg.shape}
        </span>
      </div>
