...
const ACCOUNTING_LINKS = {
  ...
  customers: erpRoute('quickbooks-customer'),
...
export default function AccountingPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Accounting"
        description="QuickBooks integration health, sync state, reconciliation"
      />
      ...
      {connected && <ActionQueue actions={actions} loading={loading} />}
      ...
      {connected && ov?.customers && ov.customers.length > 0 && (
        <SectionCard
          ...
          title="Customers in QuickBooks"
          action={`${ov.customers.length} shown`}
        >
              ...
              {ov.customers.map((customer) => (
                <tr key={customer.id}>
                  ...
                  <td className="py-2 pl-3 text-right">
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-medium ${customer.active ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600'}`}
                    >
      ...
      {connected && ov?.accounts && ov.accounts.length > 0 && (
        <SectionCard
          ...
          title="Chart of accounts in QuickBooks"
          action={`${ov.accounts.length} shown`}
        >
          <table className="w-full text-sm">
            <thead className="text-xs text-gray-500 uppercase">
              <tr className="text-left">
            ...
            <tbody className="divide-y divide-gray-100">
              {ov.accounts.map((account) => (
                <tr key={account.id}>
                  ...
                  <td className="py-2 pl-3 text-right">
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-medium ${account.active ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600'}`}
                    >
      ...
      {connected && ov?.recentInvoices && ov.recentInvoices.length > 0 && (
        <SectionCard
          id="recent-invoices"
          title="Recent invoices in QuickBooks"
          action={`Last ${ov.recentInvoices.length}`}
        >
          <table className="w-full text-sm">
            <thead className="text-xs text-gray-500 uppercase">
              <tr className="text-left">
      ...
      <SectionCard
        ...
        href={ACCOUNTING_LINKS.reconciliation}
      ...
      >
        {loading ? (
          ...
          <p className="text-sm text-gray-500">
            No reconciliation runs yet. Runs are triggered nightly + on demand.
          </p>
        ) : (
          <div className="text-sm">
            <div className="flex items-center gap-3">
              ...
              <span className="text-gray-700">
                Started {formatDateTime(m.lastReconciliation.startedAt)}
              </span>
            </div>
            {typeof m.lastReconciliation.mismatchCount === 'number' && (
              <p className="mt-2 text-gray-700">
                <span
                  className={
                    m.lastReconciliation.mismatchCount > 0
            ...
            <p className="mt-2 text-xs text-gray-400">
              {m.reconciliationRunCount} total runs in history
            </p>
          </div>
        )}
      </SectionCard>
      ...
      >
        {loading ? (
          ...
          <p className="text-sm text-gray-600">
            {m.integrationAccountsCount} QuickBooks accounts mapped to ERP entries (Income, COGS,
            Liability, Asset). Used by the invoice-sync handler when posting line items.
          </p>
        )}
      </SectionCard>

      {/* Sub-page navigation tiles */}
      <WorkspaceLinkGrid moduleKey="accounting" className="mt-6 grid max-w-2xl grid-cols-2 gap-4" />
    </div>
  );
}
      ...
      <div className="mb-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-yellow-800 mt-1">
            Sync, reconciliation, and chart-of-accounts data are unavailable until an admin
            connects.
          </p>
        </div>
        <a
      ...
      <div>
        ...
        {realmId && (
          <p className="text-xs text-green-800 mt-1">
            Realm ID: <code className="font-mono">{realmId}</code>
          </p>
        )}
      </div>
    </div>
  ...
  return (
    ...
    >
      <div className={`text-2xl font-bold ${toneClass[tone]}`}>
        {loading ? (
          <span className="inline-block h-6 w-10 bg-gray-200 rounded animate-pulse" />
        ) : (
          value
        )}
      </div>
...
function buildActionQueue(m: AccountingMetrics, connected: boolean): QueuedAction[] {
  ...
  if (pending > 0) {
    out.push({
      ...
      detail:
        'These ran into a transient error or are queued for the next batch. Usually self-resolves within an hour.',
      cta: 'View queue →',
      href: ACCOUNTING_LINKS.queue,
    });
  }
  ...
  if (!lastRecon) {
    out.push({
      ...
      detail:
        'Reconciliation compares ERP invoices to QuickBooks records and surfaces mismatches. Runs nightly automatically; trigger one now to bootstrap history.',
  ...
  if (out.length === 0) {
    out.push({
      ...
      detail:
        'All sync records are healthy and reconciliation is current. Check back tomorrow or after the next batch of work orders complete.',
