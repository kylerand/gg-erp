...
resource "aws_lambda_function" "planning_sign_off_build_package" {
  function_name    = "${var.name_prefix}-planning-sign-off-build-package"
  role             = aws_iam_role.work_orders_lambda.arn
  runtime          = "nodejs20.x"
  handler          = "sign-off-build-package.handler"
  s3_bucket        = var.lambda_artifacts_bucket_name != "" ? var.lambda_artifacts_bucket_name : null
  s3_key           = var.lambda_artifacts_bucket_name != "" ? "lambdas/work-orders-lambda.zip" : null
  filename         = var.lambda_artifacts_bucket_name == "" ? var.work_orders_lambda_zip_path : null
  source_code_hash = filebase64sha256(var.work_orders_lambda_zip_path)
...
resource "aws_apigatewayv2_integration" "planning_sign_off_build_package" {
  api_id                 = aws_apigatewayv2_api.erp.id
  integration_type       = "AWS_PROXY"
  integration_method     = "POST"
  integration_uri        = aws_lambda_function.planning_sign_off_build_package.invoke_arn
  payload_format_version = "2.0"
}

resource "aws_apigatewayv2_route" "planning_sign_off_build_package" {
  api_id             = aws_apigatewayv2_api.erp.id
  route_key          = "POST /planning/build-packages/signoffs"
  target             = "integrations/${aws_apigatewayv2_integration.planning_sign_off_build_package.id}"
...
locals {
  erp_lambdas = {
    ...
    planning_sign_off_build_package             = aws_lambda_function.planning_sign_off_build_package
