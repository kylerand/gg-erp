...
export default function AccountingLedgerPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setLoadError(null);
      try {
        const data = await listOperationalLedger(
          {
            limit: 100,
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Operational accounting entries from received POs, QuickBooks payments, and reconciliation variance."
      />
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <MetricCard
          ...
          value={formatUsdCents(summary?.sourceTotals.PAYABLE_RECEIPT.amountCents ?? 0)}
          detail={`${summary?.sourceTotals.PAYABLE_RECEIPT.count ?? 0} PO entries`}
          loading={loading}
        />
      </div>
